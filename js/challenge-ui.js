...
function chCombineShipCountObjects(shipCountMainFleet, shipCountEscort) {
	if (shipCountEscort) {
		...
		for (let key in shipCountMainFleet.c) {
			if (key == 'ids') {
				shipCountMainFleet.c.ids = shipCountMainFleet.ids.concat(shipCountEscort.ids);
				continue;
			}
			shipCountMainFleet.c[key] = shipCountMainFleet[key] + shipCountEscort[key];
		}
...
function chLoadSortieInfo(mapnum) {
	...
	let mapPath = mapdata.getMapBanner ? mapdata.getMapBanner() : 'assets/maps/'+world+'/'+mapnum+'m.png';
	// --- Custom banner ?
	if (mapdata.mapPreviewImage) mapPath = mapdata.mapPreviewImage;
